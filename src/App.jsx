...
const Header = () => (
    <header className="bg-slate-900/80 backdrop-blur-sm text-white p-4 shadow-lg shadow-yellow-500/10 border-b border-slate-800 sticky top-0 z-40">
        <div className="container mx-auto flex items-center justify-center gap-3">
            <TrophyIcon className="h-8 w-8 text-yellow-400" />
            <h1 className="text-2xl md:text-3xl font-bold tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-yellow-500">
                PORRA 1ª DIVISIÓN
            </h1>
            <span className="text-slate-400 font-light hidden sm:inline">by Zapa</span>
        </div>
    </header>
);

const Footer = () => <footer className="bg-transparent text-white text-center p-6 mt-auto"><p className="text-sm text-slate-500">Creado por ByZapa</p></footer>;

const Modal = ({ title, message, onClose }) => <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4"><Card className="w-full max-w-md text-center"><div className="p-6"><h3 className="text-2xl font-bold text-yellow-400 mb-4">{title}</h3><p className="text-slate-300 mb-6">{message}</p><button onClick={onClose} className="bg-yellow-400 text-black font-bold py-2 px-6 rounded-lg hover:bg-yellow-300 transition-all duration-300 transform hover:scale-105">Entendido</button></div></Card></div>;

const WelcomeScreen = ({ onEnter, players, selectedPlayerId, setSelectedPlayerId }) => (
    <div className="text-center p-4 max-w-2xl mx-auto animate-fade-in">
        <h2 className="text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-white to-yellow-400 mb-4">Bienvenido a la Porra</h2>
        <p className="text-slate-400 mb-8 text-lg">Selecciona tu perfil para empezar a jugar.</p>
        <Card className="w-full">
            <div className="p-6 md:p-8">
                <h3 className="text-2xl font-semibold text-white mb-4">¿Quién eres?</h3>
                <select value={selectedPlayerId} onChange={(e) => setSelectedPlayerId(e.target.value)} className="w-full p-4 bg-slate-900 border border-slate-700 rounded-lg text-white text-lg focus:outline-none focus:ring-2 focus:ring-yellow-400 mb-6 transition-all">
                    <option value="">-- Selecciona tu nombre --</option>
                    {players.map(player => (<option key={player.id} value={player.id}>{player.name}</option>))}
                </select>
                <button onClick={onEnter} disabled={!selectedPlayerId} className="w-full bg-gradient-to-r from-yellow-400 to-amber-500 text-black font-bold py-4 px-8 rounded-lg shadow-lg hover:shadow-yellow-400/20 transform hover:scale-105 transition-all duration-300 disabled:from-slate-600 disabled:to-slate-700 disabled:text-slate-400 disabled:cursor-not-allowed disabled:shadow-none">Entrar al Juego</button>
            </div>
        </Card>
    </div>
);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in">
            <div className="lg:col-span-2 space-y-6">
                <Card>
                    <div className="p-6 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            {player.status === 'activo' ? <div className="flex items-center text-green-400 font-semibold"><ShieldCheckIcon className="h-5 w-5 mr-2"/>Activo en la porra</div> : <div className="flex items-center text-red-500 font-semibold"><ExclamationCircleIcon className="h-5 w-5 mr-2"/>Eliminado en la jornada {player.jornadaEliminado}</div>}
                        </div>
                    </div>
                </Card>
                ...
                {player.status === 'activo' && (
                    <Card>
                        <div className="p-6">
                            <h3 className="text-2xl font-bold text-yellow-400 mb-4">Jornada {jornada}: Tu Elección</h3>
                            {picksClosed ? <div className="text-center p-4 bg-slate-900/50 rounded-lg"><p className="text-amber-400 text-lg">Las elecciones para la jornada {jornada} están cerradas.</p><p className="text-slate-300 mt-2">Tu elección fue: <strong className="font-bold text-white">{player.picks.find(p=>p.jornada === jornada)?.team || 'No elegiste'}</strong></p></div>
                            : hasPicked ? <div className="text-center p-4 bg-slate-900/50 rounded-lg"><p className="text-green-400 text-lg">¡Elección guardada para la Jornada {jornada}!</p><p className="text-slate-300 mt-2">Has elegido a: <strong className="font-bold text-white">{player.picks.find(p=>p.jornada === jornada).team}</strong></p></div>
                            : (<form onSubmit={handleSubmit}><p className="text-slate-400 mb-4">Elige un equipo ganador para esta jornada.</p><select value={selectedTeam} onChange={(e) => setSelectedTeam(e.target.value)} className="w-full p-3 bg-slate-900 border border-slate-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-yellow-400 transition-all"><option value="">-- Selecciona un equipo --</option>{availableTeams.map(team => <option key={team} value={team}>{team}</option>)}</select><button type="submit" disabled={!selectedTeam} className="mt-4 w-full bg-gradient-to-r from-yellow-400 to-amber-500 text-black font-bold py-3 rounded-lg shadow-lg hover:shadow-yellow-400/20 transform hover:scale-105 transition-all duration-300 disabled:from-slate-600 disabled:to-slate-700 disabled:text-slate-400 disabled:cursor-not-allowed">Confirmar Elección</button></form>)}
                        </div>
                    </Card>
                )}
                ...
                <Card>
                    <div className="p-6">
                        <h3 className="text-2xl font-bold text-yellow-400 mb-4">Historial de Elecciones</h3>
                        {player.picks.length > 0 ? <ul className="space-y-3">{player.picks.slice().reverse().map(pick => <li key={pick.jornada} className="flex justify-between items-center bg-slate-900/50 p-3 rounded-lg"><span className="text-slate-400">Jornada {pick.jornada}</span><strong className="font-semibold text-white">{pick.team}</strong></li>)}</ul> : <p className="text-slate-400">Aún no has hecho ninguna elección.</p>}
                    </div>
                </Card>
            </div>
            ...
            <div className="space-y-6">
                <Card className="text-center">
                    <div className="p-6">
                        <h3 className="text-xl font-semibold text-yellow-400 mb-2">Bote Actual</h3>
                        <p className="text-6xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-white to-yellow-400 my-2">{((players.length * 20) * 0.85).toLocaleString('es-ES', { style: 'currency', currency: 'EUR' })}</p>
                        <p className="text-sm text-slate-500 mt-2">Comisión (15%): {((players.length * 20) * 0.15).toLocaleString('es-ES', { style: 'currency', currency: 'EUR' })}</p>
                    </div>
                </Card>
                <Card>
                    <div className="p-6">
                        <h3 className="text-2xl font-bold text-yellow-400 mb-4">Jugadores Activos</h3>
                        <ul className="space-y-3">{players.filter(p => p.status === 'activo').map(p => <li key={p.id} className="flex items-center text-white bg-slate-900/50 p-3 rounded-lg"><ShieldCheckIcon className="h-5 w-5 mr-3 text-green-400" />{p.name}</li>)}</ul>
                    </div>
                </Card>
            </div>
        </div>
...
const AdminPanel = ({ gameState, onProcessJornada, onAdvanceJornada, onShowModal, onAddPlayer, onRemovePick }) => {
    ...
    return (
        <Card className="animate-fade-in">
            <div className="p-6">
                <h2 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-yellow-500 mb-6 flex items-center gap-3"><CogIcon className="h-8 w-8" />Panel de Administración</h2>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <Card className="bg-slate-900/50"><div className="p-4"><h3 className="text-xl font-bold text-yellow-400 mb-3">Añadir Jugador</h3><form onSubmit={handleAddPlayer} className="flex gap-2"><input type="text" value={newPlayerName} onChange={(e) => setNewPlayerName(e.target.value)} placeholder="Nombre del amigo" className="flex-grow p-2 bg-slate-800 border border-slate-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-yellow-400" /><button type="submit" className="bg-green-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-green-500 transition-colors">Añadir</button></form></div></Card>
                    <Card className="bg-slate-900/50"><div className="p-4"><h3 className="text-xl font-bold text-yellow-400 mb-3">Control de Jornada</h3><div className="flex flex-col sm:flex-row gap-4"><button onClick={handleProcess} disabled={picksClosed} className="flex-1 bg-blue-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-blue-500 disabled:bg-slate-600 transition-colors">Procesar Jornada {jornada}</button><button onClick={handleAdvance} disabled={!picksClosed} className="flex-1 bg-purple-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-purple-500 disabled:bg-slate-600 transition-colors">Avanzar a Jornada {jornada + 1}</button></div></div></Card>
                </div>
                
                <Card className="bg-slate-900/50 mb-6"><div className="p-4"><h3 className="text-xl font-bold text-yellow-400 mb-3">Introducir Resultados Jornada {jornada}</h3><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">{JORNADAS_DATA[jornada]?.matches.map(match => (<div key={match.id} className="flex items-center justify-between text-sm bg-slate-800 p-2 rounded-lg"><span className="text-slate-300 truncate pr-2">{match.home} vs {match.away}</span><select onChange={(e) => handleResultChange(match.id, e.target.value)} className="p-1 bg-slate-700 border border-slate-600 rounded text-white text-xs"><option value="">Ganador</option><option value={match.home}>{match.home}</option><option value={match.away}>{match.away}</option><option value="DRAW">Empate</option></select></div>))}</div></div></Card>

                <div><h3 className="text-2xl font-bold text-yellow-400 mb-4">Lista de Jugadores y Elecciones</h3><div className="overflow-x-auto"><table className="w-full text-left text-white">
                    <thead className="border-b border-slate-700"><tr className="text-slate-400 text-sm"><th className="p-3">Nombre</th><th className="p-3">Estado</th><th className="p-3">Elección (J{jornada})</th><th className="p-3 text-right">Acciones</th></tr></thead>
                    <tbody>{players.map(p => { const currentPick = p.picks.find(pick => pick.jornada === jornada); return ( <tr key={p.id} className="border-b border-slate-800 hover:bg-slate-800/40"><td className="p-3 font-semibold">{p.name}</td><td className={`p-3 font-semibold ${p.status === 'activo' ? 'text-green-400' : 'text-red-500'}`}>{p.status}</td><td className="p-3">{currentPick?.team || <span className="text-slate-500">Sin elección</span>}</td><td className="p-3 text-right">{currentPick && !picksClosed && (<button onClick={() => onRemovePick(p.id)} className="bg-red-700/80 text-white text-xs font-bold p-2 rounded-full hover:bg-red-600 transition-colors"><TrashIcon /></button>)}</td></tr> )})}</tbody>
                </table></div></div>
            </div>
        </Card>
    );
...
export default function App() {
  ...
  return (
    <div className="bg-slate-900 text-white min-h-screen font-sans flex flex-col bg-grid-slate-700/[0.05]">
      <style>{`
      ...
      <Header />
      <nav className="bg-slate-900/80 backdrop-blur-sm p-2 sticky top-[73px] z-30 mb-4">
        <div className="container mx-auto flex justify-center items-center gap-2 md:gap-6">
            <button onClick={handleGoToWelcome} className="px-4 py-2 text-sm md:text-base rounded-lg transition-colors text-slate-300 hover:bg-slate-800 hover:text-white">Inicio / Cambiar Jugador</button>
            <button onClick={() => setScreen('admin')} className="px-4 py-2 text-sm md:text-base rounded-lg transition-colors text-slate-300 hover:bg-slate-800 hover:text-white">Admin</button>
        </div>
      </nav>
